...
In all the environments, you should use your preferred package manager (e.g., `pip` or `poetry`) to manage dependencies. Ensure you install the ZenML package and any required [integrations](../../stack-components/component-guide.md).
...
2. Creating or triggering [pipeline and step build environments](../../stack-components/image-builders/) if running remotely.
3. Triggering a run in the [orchestrator](../../stack-components/orchestrators/).
...
The execution environments do not need to be built each time a pipeline is run - you can [reuse builds from previous runs to save time](../customize-docker-builds/reuse-docker-builds.md).
...
Note that even if you don't configure an image builder in your stack, ZenML still uses the [local image builder](../../stack-components/image-builders/local.md) to retain consistency across all builds. In this case, the image builder environment is the same as the client environment.
